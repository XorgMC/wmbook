...
function mb_options() {
  ...
  switch($action) {
    ...
    case "add":
      ...
      break;
    case "POST_addfk":
      setcookie("FKLastDate", $_POST['datum'], time()+3600);
    ...
    case "addfk":
      ...
      echo "<tr valign=\"top\"><th scope=\"row\"><strong>Datum</strong></th><td><input type=\"date\" name=\"datum\" id=\"datum\" value=\"" . $GDATE . "\"></td></tr>";
      echo "<tr valign=\"top\"><th scope=\"row\"><strong>Uhrzeit</strong></th><td><input type=\"time\" required min=\"00:00\" max=\"23:59\" name=\"zeitvon\" value=\"12:00\"> bis <input type=\"time\" required min=\"00:00\" max=\"23:59\" name=\"zeitbis\" value=\"14:00\"></td></tr>";
...
function show_tab_fpo() {
  ...
  echo '<table class="form-table">';
  $dapp = get_option('ferien_following') == 'TRUE' ? "WHERE KDATUM >= CURDATE() AND " : "";
  ...
  foreach($arr as $key => $row) {
    ...
    if ($PREVDATE != $row->KDATUM) {
      echo "<thead><tr><th colspan=\"2\" class=\"ws-header\">" . $TNAME[date("N", $KDM)] . ", " . date("d", $KDM) . ". " . $MNAME[date("n", $KDM)] . ". " . date("y", $KDM) . "</th></tr></thead><tbody class=\"ws-table-content\">";
      $PREVDATE = $row->KDATUM;
    }
    $TN = $row->STD_KINDER;
    ...
    echo "<tr class=\"" . $LCL . "\"><td><p class=\"ws-fpr-title\"><a href=\"?detail=" . $row->TITEL . "\">" . $row->TITEL . "</a></p><small>" . date('G:i', strtotime($row->ZEITVON)) . " - " . date('G:i', strtotime($row->ZEITBIS)) . " Uhr</small></td>";
    echo "<td>";
    echo "<input type=\"text\" value=\"" . $OVT . "\" title=\"Qty\" readonly class=\"ws-std-state $OVC\" size=\"5\">";
    echo "</td></tr>";
  }
...
function showfk( $name ) {
  ...
  echo (!empty($kurse)) ? '<h2><a href="?main" style="text-decoration: none !important; box-shadow: none;">&#x2B05;</a> ' . str_replace("!", "", $name) . '</h2>' : '<p>Es wurde(n) kein(e) ' . str_replace("!", "", $name) . ' gefunden!</p>';
  //<table class="form-table"><thead><tr><th colspan="2">' . typn($a['angebot'], TRUE) . '</th></tr></thead><tbody>
  $PRE = "<p>A";
  $POST = "<p>";
  $BESCH = "<p>findet ein Ferienkurs statt (keine Beschreibung?)</p>";
